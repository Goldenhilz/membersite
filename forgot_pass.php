...
$numrows = mysqli_num_rows($query);
if($numrows == 0){
// header("location: message.php?msg=There is no match for that username with that temporary password in the system. We cannot proceed.$u");
    exit();
} else {
...
<html>
  <head>
    ...
    response = response.replace(/[^_a-zA-Z0-9]/g, '');
    if(response == "success"){
...
<div id="main-contents" class="row">
  ...
  <form id="forgotpassform" onsubmit="return false;">
    ...
    <br /><br />
    <button class="button" id="forgotpassbtn" onclick="forgotpass()">Generate Temporary Log In Password</button> 
    <p id="status"></p>
  </form>
</div>
